...
for i in `find -name *.tif`;do convert -resize 32X32 $i $i.png_;rm $i;done
for i in `find -name *.png_`;do convert -resize 64X64 -gravity center -extent 64X64 $i $i.png;rm $i;done
mv SampleDigits "${TRAIN_DIRECTORY}"
