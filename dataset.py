...
tf.app.flags.DEFINE_string('data_dir', '/home/dan/Workspace/AI6721/data/',
...
class Dataset(object):
  ...
  def data_files(self):
    ...
    """
    if self.subset == "all":
        tf_record_pattern1 = os.path.join(FLAGS.data_dir, '%s-*' % 'train')#validation
        tf_record_pattern2 = os.path.join(FLAGS.data_dir, '%s-*' % 'validation')#validation
        data_files = tf.gfile.Glob(tf_record_pattern1)
        data_files.extend(tf.gfile.Glob(tf_record_pattern2))
